...
class AuthRepo {
  ...
  Future<ApiResponse> checkPhone(String phone) async {
    try {
      Response response = await dioClient
          .post(AppConstants.CHECK_PHONE_URI, data: {"phone": phone});
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> verifyPhone(String token) async {
    try {
  ...
  Future<ApiResponse> resetPassword(String identity, String tmpToken,
      String password, String confirmPassword) async {
    print('======Password====>$password');
    try {
      Response response =
          await dioClient.put(AppConstants.RESET_PASSWORD_URI, data: {
        "_method": "put",
        "phone": identity.trim(),
        "token": tmpToken,
        "password": password,
  ...
  Future<ApiResponse> forgetPassword(String identity) async {
    ...
    try {
      Response response = await dioClient
          .post(AppConstants.FORGET_PASSWORD_URI, data: {"phone": identity});
      return ApiResponse.withSuccess(response);
    } catch (e) {
      // print("CheckResponse2 ${e}");
