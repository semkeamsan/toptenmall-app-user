...
class OrderProvider with ChangeNotifier {
  ...
  Future<void> placeOrder(OrderPlaceModel orderPlaceModel, Function callback,
      List<CartModel> cartList, String addressID, String couponCode,
      {String bankName, String image}) async {
    _paymentModel.paymentURL = image;
    _paymentModel.bankName = bankName;
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse =
        await orderRepo.placeOrder(addressID, couponCode, _paymentModel);
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
  ...
  Future<void> uploadImage(File image, Function callback) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await orderRepo.uploadImageAPI(image);
    // _isLoading = false;
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
      String message = apiResponse.response.data['message'].toString();
      String image = apiResponse.response.data['image'].toString();
      _isLoading = false;
