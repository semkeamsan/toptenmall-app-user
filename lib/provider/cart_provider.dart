...
class CartProvider extends ChangeNotifier {
  ...
  BankInforModel _bankInforModel;
  BankInforModel get bankInfoModel => _bankInforModel;
  ...
  bool _getData = true;
  int _bankpaymentMethodIndex = 0;
  int get bankpaymentMethodIndex => _bankpaymentMethodIndex;
  String _bankpaymentName = "";
  String get bankpaymentName => _bankpaymentName;
  ...
  Future<void> getCartDataAPI(BuildContext context) async {
    _isLoading = true;
  ...
  void setSelectedShippingMethod(int index, int sellerIndex) {
    print("CheckIndexShipping $sellerIndex $index");
  ...
  Future<void> getPaymentInfor(BuildContext context) async {
    _isLoading = true;

    // notifyListeners();
    ApiResponse apiResponse = await cartRepo.getPaymentMethod();

    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
      // _cartList = [];
      print("CheckDataRepsone ${apiResponse.response.data}");
      _bankInforModel = BankInforModel.fromJson(apiResponse.response.data);
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    _isLoading = false;
  ...
  void setPaymentMethod(int index, String name) {
    _bankpaymentMethodIndex = index;
    _bankpaymentName = name;
    print("BankName $name");
    notifyListeners();
  }
}
